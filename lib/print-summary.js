function printSummary(stats) {
 ...
 console.log(`   - ${stats.userRoles.length} user-role assignments`);
 console.log(
  `   - ${stats.rolePermissions.length} role-permission assignments`
 );
 ...
 stats.roles.forEach((role) => {
  const permCount = stats.rolePermissions.filter(
   (rp) => rp.role_id === role.id
  ).length;
  const userCount = stats.userRoles.filter(
